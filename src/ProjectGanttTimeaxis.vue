<template>
	<div id="g-timeaxis">
		<div class="g-timeaxis-empty-space" :style="{ width: rowLabelWidth, background: themeColors.secondary }" />
		<div class="g-timeaxis-days" :style="{ width: `${100 - rowLabelWidth.replace('%', '')}%` }">
			<div
			...
			>
				...
				<div :style="{ background: themeColors.ternary, color: themeColors.text }">
					<div v-for="hour in day.ganttHours" :key="hour.fullDatetime" class="g-timeaxis-hour">
						<span :style="{ fontSize: hourFontSize }">{{ hour.text }}</span>
						<div class="g-timeaxis-hour-pin" :style="{ background: themeColors.text }" />
					</div>
				</div>
			</div>
		</div>
	...
	methods: {
		initAxisDaysAndHours() {
			...
			while (start.isBefore(end)) {
				let hourCountOfDay =
					start.format("DD.MM.YYYY") == end.format("DD.MM.YYYY") ? end.hour() : 24 - start.hour();
		...
		moveTimemarker(event) {
			this.timemarker.style.left =
				event.clientX - this.timemarkerOffset - this.horizontalAxisContainer.left + "px";
		},
